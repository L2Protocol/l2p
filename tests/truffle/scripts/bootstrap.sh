...
function init_genesis_data() {
     ...
     sed -i -e "s/{{NetworkId}}/${L2P_CHAIN_ID}/g" ${workspace}/storage/${node_id}/config.toml
     if [ "${node_id}" == "l2p-rpc" ]; then
...
for((i=1;i<=${NUMS_OF_VALIDATOR};i++)); do
     init_validator "l2p-validator${i}"
done
...
init_genesis_data l2p-rpc l2p-rpc

for((i=1;i<=${NUMS_OF_VALIDATOR};i++)); do
     init_genesis_data validator "l2p-validator${i}"
done

#Step 4, prepare bls wallet, used by fast finality vote
prepareBLSWallet l2p-rpc

for((i=1;i<=${NUMS_OF_VALIDATOR};i++)); do
     prepareBLSWallet "l2p-validator${i}"
done